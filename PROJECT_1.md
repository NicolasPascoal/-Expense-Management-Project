...
6. **Decorators de autorização unificados (Tarefa 1.3, 2026-07-20)**: `_autenticar()` em `utils/auth_middleware.py` é o ponto único de decode do token — `@token_required` e `@admin_required` são composições sobre ele e populam `g.user` com o payload completo (id, username, is_admin, role, empresa_id) por construção. `@non_prestador_required` (aplicado após `token_required`) bloqueia `role='prestador'` nos módulos financeiros. Nunca decodifique token fora de `_autenticar()`. Matriz de permissão completa: `docs/Authorization.md` seção 6; testes: `back/tests/test_autorizacao.py`.
