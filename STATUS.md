...
| 3.4 — Pipeline de CI/CD | [~] | 2026-08-01 | | `.github/workflows/ci.yml`: 2 jobs (`backend`, `frontend`) rodando em todo PR e push em `main`. Backend: Postgres 15-alpine efêmero como service, `ruff check` (config nova em `back/pyproject.toml`, conjunto enxuto E9+F — projeto nunca tinha sido lintado, regras de estilo completas ficariam bloqueando por motivo nenhum) + `pytest -q`. Frontend: `eslint` + `npm run build`. **Achado e corrigido no processo**: 3 imports mortos no backend (`db.py`, `modelProjetos.py`) e 54 problemas de lint no frontend — a maioria (45) eram regras experimentais de "React Compiler" do `eslint-plugin-react-hooks` (`react-hooks/refs`/`static-components`/`set-state-in-effect`/`immutability`) nunca de fato aplicáveis a este projeto (que não usa o Compiler) — desativadas com justificativa no `eslint.config.js`; os ~15 restantes eram variável/import não usado de verdade, corrigidos. Simulado localmente contra banco Postgres vazio (idêntico ao que a Action usa) antes de commitar — 50/50 testes, lint limpo, build ok. **Não cumprido ainda** (depende de decisão do usuário, não é falta de código): (a) bloqueio de merge de verdade exige regra de proteção de branch no GitHub (configuração do repositório, não arquivo de código); (b) deploy automático para staging após merge — staging roda no Umbrel/Portainer do usuário, atrás de VPN; GitHub Actions não alcança isso sem um webhook do Portainer configurado. Ambos perguntados ao usuário, aguardando decisão |
